...
D3D11_RASTERIZER_DESC g_rasterizer = {};
...
HRESULT InitDX(HWND hWnd, UINT width, UINT height, bool fullscreen)
{
	...
	g_rasterizer.FillMode = D3D11_FILL_SOLID;
	g_rasterizer.FrontCounterClockwise = true;
	for (int i = 0; i < 3; ++i)
	{
		g_rasterizer.CullMode = cull[i];
		hr = g_pDevice->CreateRasterizerState(&g_rasterizer, &g_pRasterizerState[i]);
...
{
	g_rasterizer.FillMode = fill;
	g_rasterizer.FrontCounterClockwise = true;
	for (int i = 0; i < 3; ++i)
	{
		g_rasterizer.CullMode = cull[i];
		g_pDevice->CreateRasterizerState(&g_rasterizer, &g_pRasterizerState[i]);
	}
}