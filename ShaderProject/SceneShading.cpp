...
void SceneShading::Init()
{
	...
	pModel->Load("Assets/Model/Cotoon_land.FBX");
	pModel->SetVertexShader(GetVS(VS_OBJECT));
	...
	Texture* pTex = TextureFactory::CreateFromFile("Assets/dissolve.png");
...
void SceneShading::Draw()
{
	...
	GetObj<Model>("UnityModel")->Draw();		// ƒ‚ƒfƒ‹‚Ì–¼‘O‚ðŽw’è‚µ‚Ä•`‰æ‚·‚é
	GetObj<Model>("LandModel")->Draw();
}